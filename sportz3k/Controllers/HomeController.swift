...
class HomeController: UIViewController, UINavigationControllerDelegate, GKGameCenterControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        bottomControls.refreshButton.addTarget(self, action: #selector(handleLeaderboard), for: .touchUpInside)
        bottomControls.specialButton.addTarget(self, action: #selector(handleTasks), for: .touchUpInside)
        bottomControls.superlikeButton.addTarget(self, action: #selector(handleGames), for: .touchUpInside)
...
//                let hasNotSwipedBefore = true
    ...
    @objc func handleLeaderboard() {
        let vc = GKGameCenterViewController(leaderboardID: "grp.3ksportz.alltime", playerScope: .global, timeScope: .allTime)
            vc.gameCenterDelegate = self
            present(vc, animated: true, completion: nil)
        
    }
    
    @objc func handleGames() {
        let swiftUIView = GameTabView()
        let hostingController = UIHostingController(rootView: swiftUIView)
        present(hostingController, animated: true, completion: nil)
    }
    
    @objc func handleTasks() {
        let swiftUIView = TaskListView()
        let hostingController = UIHostingController(rootView: swiftUIView)
        present(hostingController, animated: true, completion: nil)
    }
